...
from features.engineering.feature_gen.render_and_execute_templates import run_static_aggregate, render_and_run_all
from features.engineering.feature_gen.db_utils import ensure_public_tables_exist, ensure_public_table_columns_exist
...
def run(args):
    ...
    try:
        ...
        with engine.begin() as conn:
            conn.execute(text(f"DROP SCHEMA IF EXISTS data_static CASCADE;"))
            conn.execute(text(f"CREATE SCHEMA data_static;"))
            conn.execute(text(f"DROP SCHEMA IF EXISTS data_window CASCADE;"))
            conn.execute(text(f"CREATE SCHEMA data_window;"))
