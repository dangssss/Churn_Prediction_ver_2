...
```mermaid
flowchart TD
    subgraph TRIGGER["⏰ Lịch chạy: Ngày 13 & 23 hàng tháng, 09:00"]
        A["📦 ZIP files từ FTP/CSKH<br>(incoming/)"]
    end

    A --> B["DAG: ds_churn_ingest<br>━━━━━━━━━━━━━━━━━<br>1. Scan & Load ZIP → PostgreSQL<br>2. Validate data quality"]

    B --> C["DAG: ds_churn_features<br>━━━━━━━━━━━━━━━━━<br>Sliding window SQL aggregation<br>→ data_window.cus_feature_*"]

    C --> D["DAG: ds_churn_pipeline<br>━━━━━━━━━━━━━━━━━<br>8 steps (xem §3.4)"]

    D --> E{"Guardrail<br>F0.5 ≥ 0.10?<br>PR-AUC ≥ 0.05?"}
    E -->|❌ FAIL| F["⛔ Pipeline dừng<br>Ghi log lỗi"]
    E -->|✅ PASS| G{"Accept/Reject<br>new_F0.5 > prev + ε?"}

    G -->|✅ Accept| H["💾 Lưu model mới<br>(model_bundles/latest/)"]
    G -->|❌ Reject| I["♻️ Giữ model cũ"]

    H --> J["📊 Score tất cả KH active<br>bằng model MỚI"]
    I --> J2["📊 Score tất cả KH active<br>bằng model CŨ"]

    J --> K["🏁 Export Risk Table<br>→ data_static.churn_risk_predictions"]
    J2 --> K

    K --> L["👥 Bộ phận CSKH<br>truy vấn danh sách rủi ro"]

    subgraph HOUSEKEEPING["🧹 Hàng ngày 03:00"]
        M["DAG: ds_churn_housekeeping<br>Dọn bundles cũ, logs, data expired"]
    end

    style TRIGGER fill:#1a1a2e,stroke:#16213e,color:#e94560
    style HOUSEKEEPING fill:#1a1a2e,stroke:#16213e,color:#53c28b
```
