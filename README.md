...
### 1.1 Mục tiêu
Dự đoán khách hàng nào sẽ ngừng sử dụng dịch vụ trong **tháng kế tiếp** (T+1), từ đó:
...
- **y = 1** (churn): Khách hàng **không có item lẫn không có doanh thu** (`item_count == 0 AND total_fee == 0`) trong tháng T+1
- **y = 0** (active): Khách hàng có ít nhất 1 item HOẶC doanh thu > 0 trong tháng T+1
...
| CSKH evaluation files | Danh sách confirmed churners từ bộ phận Chăm sóc Khách hàng (format chính: `label_YYMM.csv`, vẫn hỗ trợ legacy `Roi_bo_MM_YY.csv`) |
...
Do giới hạn về nhãn thực tế, hệ thống trích xuất đặc trưng của tập khách hàng đã rời bỏ (Confirmed Churners) tại mốc thời gian T-1 tháng để xây dựng **Prototype**:
...
Biến đổi nghịch đảo khoảng cách thành điểm Similarity. Ngưỡng pseudo-churn không dùng giá trị cứng: hệ thống calibrate theo `similarity_quantile` của population hiện tại. Khách hàng có cấu trúc hành vi gần nhóm Churn nhất + có tín hiệu rớt số (`delta_ewma < 0`) $\rightarrow$ gán nhãn `pseudo_churn`.

### 2.5 Ước lượng Trọng số PU Learning (Positive-Unlabeled)

Confirmed CSKH là nguồn nhãn ưu tiên cao nhất. Cohort confirmed cũ được đưa vào train theo snapshot T-1; cohort confirmed mới nhất được giữ làm temporal holdout. Các nguồn phụ trợ được calibrate trọng số theo quy mô tương đối so với confirmed train và bị giới hạn bởi `min_aux_weight` / `max_aux_weight`.

| Nguồn Nhãn (Label Source) | Trọng số ($w_i$) | Nhãn mục tiêu ($y_i$) |
|-----------------------|----------------|---------------------|
| `confirmed` (Rời bỏ thật)| $1.0$ | $1.00$ |
| `pseudo_churn` (Giống rời bỏ)| calibrate động | $0.95$ |
| `reliable_neg` (Đang hoạt động)| calibrate động | $0.05$ |
| `pu_unlabeled` (Chưa rõ)| calibrate động | $0.05$ |
| `rule_based` (Outcome hoạt động thô)| calibrate động | $0.00$ hoặc $1.00$ |
...
| Guardrail | min_f0.5 = 0.10, min_pr_auc = 0.05, min_recall = 0.10 |
| Accept/Reject | New F0.5 > prev_F0.5 + ε |
| Scoring | Top 10% dynamic threshold (percentile-based) |

**Guardrail & Accept/Reject Logic:**

1. **Guardrail Check** (cổng chất lượng tối thiểu): Sau khi train, hệ thống kiểm tra F0.5 ≥ 0.10, PR-AUC ≥ 0.05 và recall ≥ 0.10. Nếu **không đạt** → pipeline dừng ngay, không deploy model.
...
- **Mount Path**: Các Pod thực thi ML mount thư mục dữ liệu từ Kind Node `/data` vào thẳng `/data` trong Container để đọc ghi file ZIP và CSKH.
...
Nhãn mục tiêu thực tế ($y_{raw}$) được định nghĩa tại đúng tháng kế tiếp ($T+1$):
- $y_{raw} = 1$ (Churn): Khách hàng **không phát sinh bất kỳ đơn hàng nào** và **không đem lại doanh thu** (`item_count == 0 AND total_fee == 0`) trong tháng $T+1$.
- $y_{raw} = 0$ (Active): Khách hàng có tối thiểu 1 đơn hàng hoặc doanh thu phát sinh $> 0$ trong tháng $T+1$.

### 8.2 Định dạng dữ liệu đầu vào CSKH
Mô-đun `cskh_loader.py` hỗ trợ nạp dữ liệu từ cả hai định dạng file phổ biến:
- Tên file quy chuẩn: `label_YYMM.csv`; vẫn hỗ trợ legacy `Roi_bo_MM_YY.csv` hoặc `Roi_bo_MM_YY.xlsx`.
...
| **File System** | | | |
| `INCOMING_DIR` | ❌ | `/data/incoming` | Thư mục chứa ZIP files đầu vào |
| `SAVED_DIR` | ❌ | `/data/saved` | Thư mục lưu data đã xử lý thành công |
| `FAIL_DIR` | ❌ | `/data/failed` | Thư mục lưu data lỗi |
| **CSKH** | | | |
| `CSKH_DIR` | ❌ | `/data/cskh` | Thư mục chứa ZIP label và file `label_YYMM.csv` từ CSKH |
...
engine = get_engine()
config = DatasetPipelineConfig(horizon_months=1)
...
|-----------|---------|--------|
| `horizon_months` | 1 | Cố định: dự đoán khách hàng không hoạt động trong tháng kế tiếp |
...
| `similarity_quantile` | 0.95 | Quantile population dùng để calibrate ngưỡng Mahalanobis similarity |
| `trend_down_quantile` | 0.25 | Quantile population dùng để calibrate tỷ lệ giảm xu hướng |
| `min_aux_weight` | 0.01 | Cận dưới trọng số cho nguồn nhãn phụ trợ |
| `max_aux_weight` | 0.80 | Cận trên trọng số cho nguồn nhãn phụ trợ |
...
| `reliable_neg_recency_quantile` | 0.25 | Quantile population dùng để calibrate reliable negative |
...
| `min_f05` | 0.10 | Guardrail: F0.5 tối thiểu |
| `min_pr_auc` | 0.05 | Guardrail: PR-AUC tối thiểu |
| `min_recall` | 0.10 | Guardrail: recall tối thiểu tại threshold đã chọn |
| `risk_top_percentile` | 10.0 | Tỷ lệ population rủi ro cao được xét theo dynamic percentile |
| `risk_max_customers` | 5000 | Giới hạn vận hành tối đa của danh sách CSKH mỗi tháng |
...
| **Saved Data** | `/data/saved/` | 90 ngày | Data đã xử lý thành công |
| **Failed Data** | `/data/failed/` | 30 ngày | Data bị lỗi |
| **Incoming Data** | `/data/incoming/` | 7 ngày | ZIP gốc chưa xử lý |
